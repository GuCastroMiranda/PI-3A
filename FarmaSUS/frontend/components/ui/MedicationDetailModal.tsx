...
export default function MedicationDetailModal({
  ...
  useEffect(() => {
    if (isVisible && medication) {
      const firstPharmacy = medication.inventories && medication.inventories.length > 0 ? medication.inventories[0].pharmacy : null;
      if (medication.cep) {
        fetchAddress(medication.cep);
      } else if (firstPharmacy) {
        setAddress(firstPharmacy.address);
      } else {
        setAddress('');
      }
    } else {
  ...
  const totalStock = medication.inventories ? medication.inventories.reduce((acc: number, curr: any) => acc + curr.quantity, 0) : 0;
  const inStock = totalStock > 0;
  const firstPharmacy = medication.inventories && medication.inventories.length > 0 ? medication.inventories[0].pharmacy : null;
  const pharmacyName = firstPharmacy ? firstPharmacy.name : 'Nenhuma unidade com estoque';

  const handleOpenMaps = () => {
    const query = encodeURIComponent(`${address}, ${pharmacyName}`);
